...
class Base extends \app\BaseController
{
     ...
     * @throws BaseException
     */
    private function checkApiPermss(): bool
    {
        if (!isset($this->ApiCheckPermss[$this->action])) {
            return true;
        }
        $LoginModel = new LoginModel;
        $permiss = $LoginModel->getLoginPermissions();
        $checkPerm=$this->ApiCheckPermss[$this->action];
        if( in_array($checkPerm, $permiss )){
            return true;
        }
        throwError('没有相关权限！');
        return false;
